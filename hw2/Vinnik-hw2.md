...
Построим гистограмммы частот для всех этих признаков:
    ...
    insurance_data %>% select('age', 'bmi', 'charges') %>% pivot_longer(1:3, values_to="value", names_to="names") %>% ggplot() + geom_histogram(aes(x = value), bins = 20, colour = "black", fill = "white") + facet_wrap(~ names, scales = "free", ncol=2)

![](Vinnik-hw2_files/figure-markdown_strict/unnamed-chunk-1-1.png)

На гистограммах выше можно отметить, что среди множеств значений `bmi` и
...
-   различия между мужчинами и женщинами по непрерывному признаку
    расходы (charges) являются значимыми
-   различия по остальным признакам: возраст (age), индекс массы тела
    (bmi) не являются значимыми

## Анализ коррелляций непрерывных признаков

Посмотрим на корреляции непрерывных признаков.

    library(corrplot)

    ## corrplot 0.92 loaded

    plotCor <- corrplot(cor(insurance_data %>% select('age','bmi','charges')))
